...
still lookin' good. These settings shouldn't affect functionality.
...
originally opened to scratch by default.
...
I would prefer to follow a file's indenting style instead of enforcing my
own, if possible. =dtrt'indent= works for most mainstream languages.
...
These are global bindings or packages that enhance/effect binding keys.
...
=C-+= and =C--= to resize text like every other program does.
...
This package automatically hides preceding asterisks, and makes the leading one
pretty.
...
Magit is so good that it doesn't require any configuration.
...
*** [g]o [s]ayonara (vim-sayonara inspired)
...
*** evil-unimpaired (inspired by tpope's unimpaired)
...
A "leader" key is a concept from Vim, and it's just a global prefix key like
...
Setup key bindings for various modes to "be more evil."
*** ibuffer
