...
A nice, light theme. Currently my default.
...
A nice, dark theme. I use this when the light is too bright.
...
are changes to update. Only run it when =config.el= is missing. (see [[file:init.el::%3B%3B%20`org-babel-load-file`%20increases%20startup%20time,%20so%20only%20do%20it%20if%20necessary.][init.el]])
...
Toggle visibility of details, as well as other small improvements.
